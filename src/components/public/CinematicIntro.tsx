...
export const CinematicIntro: React.FC = () => {
  ...
  // Smooth physics spring for cinematic realism
  ...
  const stageScale = useTransform(smoothProgress, [0, 0.4, 0.8, 1], [1, 1.08, 1.15, 1.18]);
  const stageY = useTransform(smoothProgress, [0, 0.5, 1], ['0%', '-3%', '-6%']);

  // 2. Darkness overlay (starts almost pitch black, lifts as lights focus)
  const darknessOpacity = useTransform(smoothProgress, [0, 0.15, 0.45, 0.85], [0.94, 0.85, 0.35, 0.2]);

  // 3. Left Spotlight rotation & intensity
  // Origin top-left: starts angled away, sweeps smoothly to center stage
  const leftSpotlightRotate = useTransform(smoothProgress, [0, 0.12, 0.45], [-35, -28, -6]);
  const leftSpotlightOpacity = useTransform(smoothProgress, [0, 0.1, 0.3, 0.8], [0.35, 0.65, 0.95, 0.8]);
  const leftSpotlightScale = useTransform(smoothProgress, [0, 0.35], [0.85, 1.15]);

  // 4. Right Spotlight rotation & intensity
  // Origin top-right: starts angled away, sweeps to center stage
  const rightSpotlightRotate = useTransform(smoothProgress, [0, 0.12, 0.45], [35, 28, 6]);
  const rightSpotlightOpacity = useTransform(smoothProgress, [0, 0.1, 0.3, 0.8], [0.35, 0.65, 0.95, 0.8]);
  const rightSpotlightScale = useTransform(smoothProgress, [0, 0.35], [0.85, 1.15]);

  // Center stage illuminated pool of light
  const centerPoolOpacity = useTransform(smoothProgress, [0.15, 0.4, 0.7], [0, 0.8, 1]);
  const centerPoolScale = useTransform(smoothProgress, [0.15, 0.45], [0.5, 1.2]);

  // 5. Cherry blossom particles inside lights
  const blossomOpacity = useTransform(smoothProgress, [0.25, 0.45, 0.85], [0, 0.85, 0.4]);
  const blossomY = useTransform(smoothProgress, [0.25, 0.85], ['-20px', '120px']);

  // 6. Moon reveal behind the stage
  const moonOpacity = useTransform(smoothProgress, [0.38, 0.58], [0, 1]);
  const moonScale = useTransform(smoothProgress, [0.38, 0.65, 0.85], [0.75, 1, 1.05]);
  const moonGlow = useTransform(
    smoothProgress,
    [0.45, 0.65, 0.8],
    [
      '0 0 30px rgba(192, 78, 207, 0.3)',
      '0 0 70px rgba(224, 102, 255, 0.6)',
      '0 0 100px rgba(212, 175, 55, 0.8)',
    ]
  );

  // 7. Eclipse Sequence
  // Dark celestial shadow crosses moon disc
  const eclipseShadowX = useTransform(smoothProgress, [0.55, 0.72, 0.82], ['-110%', '-25%', '0%']);
  const eclipseCoronaOpacity = useTransform(smoothProgress, [0.65, 0.76, 0.86], [0, 0.85, 1]);

  // 8. Memoria'26 Title reveal in exact layers
  // Layer 1: "THE ECLIPSE"
  ...
  // Layer 3: "Memoria'26" (Great Vibes, blur-to-sharp, gold/magenta bloom)
  const wordmarkOpacity = useTransform(smoothProgress, [0.82, 0.89], [0, 1]);
  const wordmarkScale = useTransform(smoothProgress, [0.82, 0.89], [0.9, 1]);
  ...
  // 10. Initial Scroll Prompt (visible only at the very beginning)
  ...
  return (
    <div ref={containerRef} className="relative h-[320vh] w-full bg-[#0D0518]">
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center select-none">
        {/* Layer 1: Stage Background Scene */}
        <motion.div
        ...
        {/* Layer 3: Moon and Eclipse Layer behind stage center */}
        <div className="absolute top-[18%] sm:top-[20%] flex items-center justify-center pointer-events-none z-10">
          <motion.div
            style={{
              opacity: moonOpacity,
              scale: moonScale,
            ...
            className="relative w-44 h-44 sm:w-64 sm:h-64 rounded-full overflow-hidden flex items-center justify-center will-change-transform"
          >
            {/* Luminous Moon Texture */}
            <img
            ...
            <motion.div
              ...
              className="absolute inset-0 w-full h-full rounded-full bg-[#0D0518]/95 shadow-[inset_0_0_20px_#000]"
            />
        ...
        {/* Layer 4: Volumetric Theatrical Left Spotlight */}
        <motion.div
          style={{
          ...
          className="absolute -top-10 -left-10 w-[70vw] sm:w-[50vw] h-[140vh] pointer-events-none z-20 mix-blend-screen will-change-transform"
        >
          <img
        ...
        {/* Layer 5: Volumetric Theatrical Right Spotlight */}
        <motion.div
          style={{
          ...
          className="absolute -top-10 -right-10 w-[70vw] sm:w-[50vw] h-[140vh] pointer-events-none z-20 mix-blend-screen will-change-transform"
        >
          <img
            src="/assets/spotlight-beam.png"
        ...
        {/* Layer 7: Cherry Blossom Petals drifting across illuminated zone */}
        <motion.div
        ...
        <div className="relative z-30 flex flex-col items-center justify-center text-center px-4 max-w-5xl">
          ...
          {/* Phase 8: Hero Meta Info & CTA (Only reveals after the cinematic eclipse) */}
          <motion.div
        ...
        {/* Initial Scroll Prompt (Only visible at top) */}
        <motion.div
